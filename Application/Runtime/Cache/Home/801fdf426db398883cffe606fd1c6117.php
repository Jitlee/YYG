...
<ul id="recrodList" class="mui-table-view">
	<?php if(is_array($list)): foreach($list as $key=>$item): ?><li class="mui-table-view-cell">
			<span class="money"><?php echo ($item["money"]); ?>
			<?php if($item["flag"] == 2): ?>(立即揭拍)<?php endif; ?>
			</span>
...
<script>
	$(function() {
		...
		function page() {
			$.get("<?php echo U('prizerecord', '', '');?>/<?php echo ($gid); ?>/<?php echo ($uid); ?>/" + (++pageNum), null, function(list) {
