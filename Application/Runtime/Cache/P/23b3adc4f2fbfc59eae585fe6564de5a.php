...
<title>支付_<?php echo config("web_name");?></title>
...
<div class="logo">
	<div class="float">
		<span class="logo_pic"><a href="/index.php/P" class="a" title="{wc:fun:_cfg("web_name")}">
...
<div class="shop_payment">
	...
	<div class="payment_Con">
		<ul class="order_list">
			...
			<?php if(is_array($list)): foreach($list as $key=>$item): ?><li class="end">
				<span class="name">
               		<a class="blue" href="<?php echo U('Index/view', '', '');?>/<?php echo ($item["gid"]); ?>"><?php echo ($item["good"]["title"]); ?></a>
                </span>
				<span class="moneys"><?php echo ($item["good"]["money"]); ?></span>
				<span class="money"><span class="color">￥<b><?php echo ($item["good"]["danjia"]); ?></b></span></span>
				<span class="num orange Fb"><?php echo ($item["count"]); ?></span>
				<span class="all"><?php echo ($item['count'] * $item['good']['danjia']); ?></span>
			</li><?php endforeach; endif; ?>         
			<li class="payment_Total">
				<div class="payment_List_Lc">
					<a href="<?php echo U('Cart/index', '', '');?>" class="form_ReturnBut">返回购物车修改订单</a>
				</div>
				<p class="payment_List_Rc">商品合计：<span class="orange F20"><?php echo ($total); ?></span> 元</p>
			</li>
			<!-- 福分 -->
            <?php if($account["score"] > 0): ?><li id="liPayByPoints" class="point_out">
			 		<div class="payment_List_Lc">
						<input type="checkbox" class="input_choice" id="scoreCheckBox" name="shop_score" value="1"/>使用福分抵扣现金：您的福分(<?php echo ($account["_score"]); ?>)本次消费最多可抵扣现金
	                    <span class="orange Fb"><?php echo min($account['score'] / 100, $total);?></span>元)
                   	</div>
             ...
             <?php else: ?>
              <li id="liPayByPoints" class="point_out point_gray">
					<div class="payment_List_Lc">
						<input type="checkbox" class="input_choice" disabled="disabled"/>使用福分抵扣现金：您的福分(<?php echo ($account["_score"]); ?>)本次消费最多可抵扣现金
                   	</div>
                    <span class="orange Fb"><?php echo ($account['score'] / 100); ?></span>元)
              </li><?php endif; ?>
             <!-- 福分 -->
            <!-- 余额支付 start-->
