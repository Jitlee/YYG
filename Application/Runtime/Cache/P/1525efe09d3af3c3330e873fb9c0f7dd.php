...
<!--content left end-->
	...
	<div class="R-content">
		...
		<div class="last">
			<ul>
				<li><h3>可用积分：</h3><b class="orange"><?php echo ($data["score"]); ?></b>	</li>
				<li>			
					<h3>可用金钱：</h3>	<b class="orange">
					
					<?php echo ($data["score"]); ?>积分 / 100积分<h3>(1元=100积分)</h3>	 = <?php echo ($data["score"]/100); ?>元<h3>(取整)</h3>	
					</b>
				</li>
			</ul>
		</div>
...
<script>
	$(function(){ 
		...
		function pageAll(clear) {
			...
			$.get("<?php echo U('pagescore', '', '');?>/20/" + (++pageNumfinish), {
